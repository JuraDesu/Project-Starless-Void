...
$c e_update(c_aabb, c_rigid_body& rb, c_player, c_collider){
...
$r e_add(const c_presented_motion& motion, c_bounds) {
...
$c g_start {
    ...
    world.spawn<p_player>([](entity& e) {
        e.set<c_rigid_body>({{bounds.pos.x + bounds_center.x, bounds.pos.y + bounds.bounds.y * 0.2}, 0.0f, {}, 0.0f});
    });
    world.spawn<p_enemy>([](entity& e) {
        e.add<c_enemy_boss>();
        e.set<c_rigid_body>({{bounds.pos.x + bounds_center.x, bounds.pos.y + bounds.bounds.y * 0.9}, 0.0f, {}, 0.0f});
    });
    world.spawn<p_enemy>([](entity& e) {
        e.add<c_dummy>() && e.set<c_rigid_body>({{bounds.pos.x + bounds.bounds.x * 0.2, bounds.pos.y + bounds.bounds.y * 0.3}, 0.0f, {}, 0.0f});
    });
